...
<script>
	  ...
	  $("#rss-default").rss("http://feeds.folha.uol.com.br/ciencia/rss091.xml").show();

		$("#rss-styled").rss("http://feeds.folha.uol.com.br/ciencia/rss091.xml", {
			limit: 5,
			layoutTemplate: '<dl class="dl-horizontal">{entries}</dl>',
			entryTemplate: '<dt><a href="{url}">{title}</a></dt><dd>{shortBodyPlain}[{author}@{date}]</dd>'
		}).show();

		$("#rss-metro").rss("http://feeds.folha.uol.com.br/ciencia/rss091.xml", {
			limit: 4,
